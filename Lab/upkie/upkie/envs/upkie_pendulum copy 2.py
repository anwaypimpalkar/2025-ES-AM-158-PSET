...
class UpkiePendulum(gym.Wrapper):
    ...
    only affect wheel velocities. This way, it behaves like a
    ...
    def __init__(
        ...
        # Limit episode length to avoid super long rollouts
    ...
    def step(
        ...
        if abs(position) > 3.0:
            logger.warning("Position limit reached (pos=%.2f m)", position)
            terminated = True
