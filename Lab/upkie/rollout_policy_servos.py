...
class ServoVelActionWrapper(gym.ActionWrapper):
    ...
    def action(self, action):
        ...
        # Legs -> position (map [-1,1] -> [low, high])
        if self._n_legs > 0:
            leg_pos_cmd = self.leg_pos_low + (a_leg + 1.0) * 0.5 * (
                self.leg_pos_high - self.leg_pos_low
            )
        else:
            leg_pos_cmd = np.zeros(0, np.float32)
...
class ServoObsFlattenWrapper(gym.ObservationWrapper):
    ...
    def observation(self, observation):
        ...
        for j in self.joint_names:
            vec += [
                float(observation[j]["position"][0]),
                float(observation[j]["velocity"][0]),
            ]
        return np.asarray(vec, dtype=np.float32)

class FallTerminationWrapper(gym.Wrapper):
    """
    Terminates the episode when the base pitch exceeds a threshold.
    Uses info["spine_observation"]["base_orientation"]["pitch"].
    """

    def __init__(self, env: gym.Env, fall_pitch: float = 1.0):
        super().__init__(env)
    ...
    def step(self, action):
        ...
        spine_obs = info.get("spine_observation", None)
        if spine_obs is not None:
            pitch = spine_obs["base_orientation"]["pitch"]
...
def make_wrapped_env(
    *,
    ...
    env = ServoVelActionWrapper(env, fixed_order=fixed_order, gains=gains)
    env = ServoObsFlattenWrapper(env)
    env = FallTerminationWrapper(env, fall_pitch=1.0)  # <-- add this line
    env = TimeLimit(env, max_episode_steps=max_steps)
    return env
...
def rollout(
    ...
    print(f"[ROLLOUT] Loading model from: {model_path}")
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Rollout SB3 PPO policy on Upkie-Spine-Servos."
    )
    p.add_argument(
        "--model",
        type=str,
        required=True,
        help="Path to .zip model file (e.g., ./models/servos_best/best_model.zip)",
    )
    p.add_argument(
